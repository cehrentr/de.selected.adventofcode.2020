class AdventOfCodePuzzleDay01:
    ...
    TARGET_EXPENSE_SUM = 2020

    def __init__(self, puzzle_input: list):
        self.expenses = puzzle_input

    def solve_puzzle_1(self) -> int:
        for exp in self.expenses:
            diff = self.TARGET_EXPENSE_SUM - exp

            if diff in self.expenses:
                return exp * diff

    def solve_puzzle_2(self) -> int:
        for exp1 in self.expenses:
            for exp2 in self.expenses:
                diff = self.TARGET_EXPENSE_SUM - exp1 - exp2

                if diff in self.expenses:
                    return exp1 * exp2 * diff
