class AdventOfCodePuzzleDay13:
    ...
    An x in the schedule means there are no constraints on what bus IDs must depart at that
    time.
    ...
    def __init__(self, puzzle_input: list):
        bus_notes = puzzle_input
        self.departure_timestamp = int(bus_notes[0])
        self.bus_ids = bus_notes[1].split(",")

    def solve_puzzle_1(self) -> int:
        result = 0
        timestamp = self.departure_timestamp

        while result == 0:
            for bus_id in [int(bus_id) for bus_id in self.bus_ids if bus_id != "x"]:
                if timestamp % bus_id == 0:
                    result = (timestamp - self.departure_timestamp) * int(bus_id)
                    break
            timestamp += 1
    ...
    def solve_puzzle_2(self) -> int:
        ...
        while result == 0:
            ...
            for bus_id in self.bus_ids:
                if bus_id != "x" and timestamp % int(bus_id) == 0:
                    bus_ids_departing.append(bus_id)
