...
class AdventOfCodePuzzleDay09:
    ...
    Suppose the 26th number is 45, and the first number (no longer an option, as it is
    more than 25 numbers ago) was 20. Now, for the next number to be valid, there needs
    ...
    def __init__(self, puzzle_input: list):
        self.preamble = [int(item) for item in puzzle_input]

    def solve_puzzle_1(self, preamble_length: int) -> int:
        return self.__find_weaknumber(preamble_length=preamble_length)

    def solve_puzzle_2(self, preamble_length: int) -> int:
        result = -1
        weaknumber = self.__find_weaknumber(preamble_length=preamble_length)
        weaknumber_idx = self.preamble.index(weaknumber)

        for idx, item in enumerate(self.preamble[:weaknumber_idx]):
            subset = self.preamble[idx:weaknumber_idx]

            for range_size in range(2, len(subset)+1):
                testset = subset[0:range_size]

                if sum(testset) == weaknumber:
                    return min(testset) + max(testset)

        return result

    def __find_weaknumber(self, preamble_length: int) -> int:
        result = -1
        idx = preamble_length
        range_start = idx - preamble_length
        range_end = range_start + preamble_length + 1

        for item in self.preamble[preamble_length:]:
            combination_sums = set([
                self.__get_combination_sum(item) for item in combinations(set(self.preamble[range_start:range_end]), 2)
            ])

            if item not in combination_sums:
                result = item
                break

            idx += 1
            range_start, range_end = (idx - preamble_length), (range_start + preamble_length + 1)

        return result

    @staticmethod
    def __get_combination_sum(combination: Tuple) -> int:
