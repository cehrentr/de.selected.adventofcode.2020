...
class AdventOfCodePuzzleDay10:
    ...
    seat produces the wrong number of jolts. Always prepared, you make a list of all
    ...
    outlet to your device. The previous rules about when adapters can successfully connect still
    apply.
    ...
    def __init__(self, puzzle_input: list):
        self.adapters = puzzle_input

    def solve_puzzle_1(self) -> int:
        # Add seat adapter joltage
        adapters = sorted(self.adapters + [0])

        jolt_differences = []
        for i, item in enumerate(adapters):
            next_item = adapters[i + 1] if i + 1 < len(adapters) else adapters[i] + 3
            jolt_differences.append(next_item - adapters[i])
        jolt_differences = Counter(jolt_differences)
    ...
    def solve_puzzle_2(self) -> int:
        ...
        for adp in reversed(adapters[:-1]):
            arrangements[adp] = sum(arrangements.get(x, 0) for x in (adp + 1, adp + 2, adp + 3))
