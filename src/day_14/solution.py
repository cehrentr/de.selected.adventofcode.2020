...
class AdventOfCodePuzzleDay14:
    ...
    program on the ferry, so the docking parameters aren't being correctly initialized
    ...
    bit (representing 2^35) on the left and the least significant bit (2^0, that is, the 1s bit)
    on the right. The current bitmask is applied to values immediately before they are written
    to memory: a 0 or 1 overwrites the corresponding bit in the value, while an X leaves the
    bit in the value unchanged.
    ...
    overwrite two bits in every written value: the 2s bit is overwritten with 0, and the
    64s bit is overwritten with 1.
    ...
    This time, the mask has no effect, as the bits it overwrote were already the values the
    mask tried to set. Finally, the program tries to write 0 to address 8:
    ...
    def __init__(self, puzzle_input: list):
        self.program = [tuple(item.split(" = ")) for item in puzzle_input]

    def solve_puzzle_1(self) -> int:
        mem = {}
        mask = ""

        for action, value in self.program:

            if action == "mask":
                mask = value
            elif action[:3] == "mem":
                address = action[3:]
                mem[address] = int(self.__calc_binary_value(mask=mask, value=value), 2)
            else:
                raise ValueError

        return sum([mem[key] for key in mem.keys()])
    ...
    def solve_puzzle_2(self) -> int:
        ...
        for action, value in self.program:
            ...
            if action == "mask":
                ...
                address = action[4:-1]
                possible_addresses = list(self.__get_possible_addresses(mask=mask, address=address))
                # print(f"address={address}; possible_addresses={possible_addresses}")

                for address in possible_addresses:
                    # print(f"address={address}; address={int(address, 2)}; value={value}")
                    # mem[int(address, 2)] = int(self.__calc_binary_value(mask=mask, value=value), 2)
    ...
    def __calc_binary_value(self, mask: str, value: int) -> str:
        bit_value = ("{0:036b}".format(int(value)))
        result = ""

        for i in range(self.BIT_LENGTH):
            result += bit_value[i] if mask[i] == "X" else mask[i]

        return result

    def __get_possible_addresses(self, mask: str, address: int):
        address_mask = ""
        bit_address = ("{0:036b}".format(int(address)))

        for i in range(self.BIT_LENGTH):
            address_mask += bit_address[i] if mask[i] == "0" else mask[i]

        # print(f"bit_address={bit_address}; address_mask={address_mask}")

        possible_addresses = self.__get_address_mask_permuatations(mask=address_mask)
        # print(f"possible_addresses={possible_addresses}")

        return self.__flatten_addresses(possible_addresses)

    def __get_address_mask_permuatations(self, mask: str):
        result = []

        if "X" not in mask:
            return mask
        else:
            result.append(self.__get_address_mask_permuatations(mask.replace("X", "0", 1)))
            result.append(self.__get_address_mask_permuatations(mask.replace("X", "1", 1)))

        return result

    def __flatten_addresses(self, addresses: Iterable):
        for address in addresses:
            if isinstance(address, Iterable) and not isinstance(address, (str, bytes)):
                yield from self.__flatten_addresses(address)
            else:
                yield address
